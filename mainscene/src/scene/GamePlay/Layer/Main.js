var GPMainLayer = cc.Layer.extend({
    ...
    loadProperty : function(){
        ...
        this.money=GameManager.getGold();//加载金币
    ...
    loadNextGroupMonster : function(){
        ...
        this.monsterblood[GameManager.getGroup() - 1]=[];
    ...
    loadDeleteTower : function(x, y,result){
        var node = null;
        if (this.deleteTower != null) {
            node = this.deleteTower;
            //node.stopAllActions();
            //node.setOpacity(255);
        }else{
            node = new ccui.ImageView("res/UI/SELL_96.png");
            node.setTouchEnabled(true);
            node.setSwallowTouches(false);
            node.addTouchEventListener(function () {
                var result = this.getInfoFromMapByPos(x, y);
                name=result.row*10+result.cel;
                cc.log(name.toString())
                this.removeChildByTag(name.toString());
                this.tiledMapRectArrayMap[result.row][result.cel]=0 ;
                this.money+=50;
                var scene=cc.director.getRunningScene();
                var layer=scene.uiLayer;
                layer.goldText.setString(this.money);
                this.removeChild(this.deleteTower);
                this.deleteTower = null;
               // this.removeTowerPanel();
                this.towerPanel=null;
            }.bind(this));
            this.addChild(node);
            this.deleteTower = node;
        }
...
loadTouchWarning : function(x, y){
    ...
    createMonster : function(){
        ...
        var bg=new ccui.LoadingBar("res/loading-bar-bg.png");
        bg.setDirection(ccui.LoadingBar.TYPE_LEFT);
        bg.setPercent(100);
        var scale=node.width/700;
        bg.setScale(node.width/700);
        bg.setPosition(node.width/2,node.height);
        node.addChild(bg);
        var loadbar=new ccui.LoadingBar();
        loadbar.setName("loadbar");
        loadbar.loadTexture("res/loading-bar.png");
        loadbar.setDirection(ccui.LoadingBar.TYPE_LEFT);
        loadbar.setPercent(100);
        var scale=node.width/700;
        loadbar.setScale(node.width/700);
       // loadbar.setContentSize();
        loadbar.setPosition(node.width/2,node.height);
        node.addChild(loadbar);
        this.addChild(node, this.ZOrderEnum.MONSTER);
        GameManager.currMonsterPool[GameManager.getGroup() - 1].push(node);
        this.monsterblood[GameManager.getGroup() - 1].push(node.blood);
    ...
    createTower : function(data){
        ...
        switch (data.name){
            ...
            case "Shit":
                towerData.scope = 300;
                towerData.bulletSpeed = 40;
                towerData.bulletAttack=4;
                towerData.cost=120;
                node = new Shit(towerData);
                this.money-=towerData.cost;
                var scene=cc.director.getRunningScene();
                var layer=scene.uiLayer;
                layer.goldText.setString(this.money);
            ...
            towerData.scope = 300;
            towerData.bulletSpeed = 40;
            towerData.bulletAttack=3;
            towerData.cost=150;
            node = new Fan(towerData);
            this.money-=towerData.cost;
            var scene=cc.director.getRunningScene();
            var layer=scene.uiLayer;
            layer.goldText.setString(this.money);
        ...
        for (var x = 0; x < GameManager.currBulletPool.length; x++) {
            ...
            for (var y = 0; y < GameManager.currMonsterPool.length; y++) {
                for (var z = 0; z < GameManager.currMonsterPool[y].length; z++) {
                    ...
                    enemycurrent = this.monsterblood[y][z];
                    //var data1 = GameManager.currMonsterDataPool[y][z];
                    ...
                    if (cc.rectContainsPoint(enemyRect, bullet.getPosition())) {
                        var flag=0;
                        if(bullet.getName()=="PShit")
                        {
                            var shit=new cc.Sprite("#PShit-12.png");
                            enemy.speed=80;
                            shit.setPosition(enemy.width/2,enemy.height-enemy.height+30);
                            enemy.addChild(shit);
                        }

                        //cc.log(enemycurrent);
                        //sum-=bullet.getTag()
                        enemy.blood-=bullet.getTag();

                       // bullet.
                        if( enemy.blood<0)
                            enemy.blood=0;
                        var loabar=ccui.helper.seekWidgetByName(enemy,"loadbar");
                        loabar.setPercent(enemy.blood*1.0/enemycurrent*100);
                        //cc.log(enemy.blood);
                       // cc.log(data1.blood);

                        // 移除子弹
                        if(bullet.getName()!="PFan")
                        {
                        ...
                        {
                           //cc.log(sum);in
                            this.money+=parseInt(enemycurrent);
                            var scene=cc.director.getRunningScene();
                            var layer=scene.uiLayer;
                            layer.goldText.setString(this.money);
    ...
    onGameOver : function(event){
        ...
        var level = GameManager.getLevel() + 1;
        switch (level){
            case 1:cc.sys.localStorage.setItem(Config.ISTWO_UNLOCK_KEY,"YES");break;
            case 2:cc.sys.localStorage.setItem(Config.ISTHERR_UNLOCK_KEY,"YES");break;
            case 3:cc.sys.localStorage.setItem(Config.ISFOUR_UNLOCK_KEY,"YES");break;
            case 4:cc.sys.localStorage.setItem(Config.ISFIVE_UNLOCK_KEY,"YES");break;
            default:
                cc.log("超过限定关卡！");
                break;
        }
    ...
    onCreateTower : function(event){
        ...
        var result = self.getInfoFromMapByPos(data.x, data.y);
        var Name=result.row*10+result.cel;
        cc.log(Name.toString())
        //cc.log(pos.toString());
        node.setTag(Name.toString());
       // cc.log(data.x*10+data.y);
        self.addChild(node);
    ...
    onTouchEnded : function (touch, event) {
        ...
        if (!result.isInMap) {
            ...
            var loaction=self.tiledMapRectArrayMap[result.row][result.cel];
            // 已经有塔或者障碍物
            //if (self.tiledMapRectArrayMap[result.row][result.cel] != self.tiledMapRectMapEnemu.NONE) {
            if (loaction == self.tiledMapRectMapEnemu.TOWER){
                cc.log();
                self.loadDeleteTower(touch.getLocation().x, touch.getLocation().y,result);
                    //self.removeChild(self.getChildByName("Tower"));
                //self.loadTouchWarning(result.x + self.tileSize.width / 2, result.y + self.tileSize.height / 2);
            }else if (loaction!=self.tiledMapRectMapEnemu.NONE&&loaction!=self.tiledMapRectMapEnemu.ROAD){
                // 当前位置没有塔和障碍物
                if(self.deleteTower!=null)
                {
                    self.removeChild(self.deleteTower);
                    self.deleteTower=null;
                }
