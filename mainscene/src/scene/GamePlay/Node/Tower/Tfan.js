var Fan = TowerBase.extend({
    ctor : function(data){
        this._super("#Fan-13.png", data);
        // 0.5 秒开火一次
        this.schedule(this.onRotateAndFire, 0.5);
    },
    loadWeapon : function(){
        var node = new cc.Sprite("#PFan31.png");
        node.setScale(1.3);
        this.addChild(node);
        this.weapon = node;
    ...
    onRotateAndFire : function(){
        ...
        if (nearestEnemy != null){
            ...
            var action = cc.sequence(callBack);
            this.weapon.runAction(action);
        }
    },
    onFire : function(){
        var currBullet = this.createBullet();
        currBullet.setScale(1.3);
        var frames=[];
        for(var i=1;i<=3;++i)
        {
            var file=cc.spriteFrameCache.getSpriteFrame("Fan3"+i+".png");
            frames.push(file);
        }
        var animation=new cc.Animation(frames,0.2);
        var animate=new cc.animate(animation);
        currBullet.runAction(animate.repeatForever());
